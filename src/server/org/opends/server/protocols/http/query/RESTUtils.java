...
import static org.opends.server.authorization.dseecompat.Aci.ZERO_OR_MORE_WHITESPACE;
...
public class RESTUtils {
  ...
  public static Response delete(String dn, MultivaluedMap<String,
      String> queryParams, ClientConnection conn) throws DirectoryException {
    DeleteOperation delOp;
    DN baseDN = DN.decode(dn);
    SearchOperation op = getSearchOperation(queryParams, conn, baseDN);
    ...
    AccessControlHandler<?> handler = AccessControlConfigManager
    .getInstance().getAccessControlHandler();
    HTTPDeleteOperation delOperation = new HTTPDeleteOperation(
        new DeleteOperationBasis(conn, 0, 0, null, e.getDN()), e);
  ...
  public static SearchOperation getSearchOperation(MultivaluedMap<String,
      String> queryParams, ClientConnection conn,
      String dnParam) throws DirectoryException {
    DN baseDN = DN.decode(queryParams.getFirst(dnParam));
    return getSearchOperation(queryParams, conn, baseDN);
  }

  private static SearchOperation getSearchOperation(
      MultivaluedMap<String, String> queryParams, ClientConnection conn,
      DN baseDN) throws DirectoryException {
